...
// Reusable class string - avoids repaeating long className strings
...
export interface FormState {
    ...
    isSold: boolean;
    created_at: string;
    images: string[];  //Supabase storage public URLs -> saved to DB
    localImages: string[]; //DEvice URIs -> shown in preview only
}
...
export default function Create() {
    ...
    const handleSubmit = async () => {
        ...
        if (priceNum > MAX_PRICE)
            return Alert.alert(
                "Validation", `Price cannot exceed ₹ ${MAX_PRICE.toLocaleString("en-EN")}`)
        ...
        const { error } = await authSupabase.from("properties").insert({
            ...
            longitude: form.latitude ? Number(form.longitude) : null,
            images: form.images,
            is_features: form.isFeatured,
            is_sold: false
                ...
                >
                        ...
                        <View className='flex-row flex-wrap gap-3'>
                            {form.localImages.map((uri, index) => (
                                <View key={index} className='relative'>
                                    ...
                                    />
                                    {index === 0 && (
                                        <View className='absolute top-1 left-1 bg-blue-600 px-1.5 py-0.5 rounded-ful'>
                            ...
                            {form.localImages.length < 6 && (
                                ...
                                >
                                    {uploadingImages ? (
                                        ...
                                        <>
                                            <Ionicons
                                                name='camera-outline' size={22} color={"#9ca3af"} />
                    ...
                    <View className={sectionClass}>
                        ...
                        <TextInput className={inputClass}
                            placeholder='e.g Modern 3 BHK in Bandra'
...
const Toggle = ({
    ...
    <TouchableOpacity
        ...
        <View className={`w-6 h-6 rounded-full border-2 items-center justify-center ${value ? "bg-blue-600 border-blue-600" : "border-gray-300"
            }`}>
